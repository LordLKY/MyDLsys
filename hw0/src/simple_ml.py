...
def parse_mnist(image_filename, label_filename):
    ...
    ### BEGIN YOUR CODE
    # pass
    data_folder = '/content/drive/MyDrive/10714/hw0/'
...
def softmax_loss(Z, y):
    ...
    Zw = np.log(np.sum(np.exp(Z), axis=1))
    return np.average(Zw - Zy)
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(NUM // batch):
      ...
      G2 = np.dot(Z1, W2)  # [b, class]
      G2 -= np.amax(G2, axis=1).reshape([num, 1])
      G2 = np.exp(G2)
      Iy = np.zeros_like(G2, dtype=np.float32)
      Iy[np.arange(Iy.shape[0]), y0] = 1
      G2 = G2 / np.sum(G2, axis=1).reshape([num, 1]) - Iy

      G1 = (Z1 > 0) * np.dot(G2, W2.T)  # [b, h]

      delta1 = np.dot(X0.T, G1) / batch
      delta2 = np.dot(Z1.T, G2) / batch
      W1 -= lr * delta1
      W2 -= lr * delta2
    ### END YOUR CODE
