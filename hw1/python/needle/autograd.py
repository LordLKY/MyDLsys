...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      ...
      for i in range(0, len(node.inputs)):
        sub_node = node.inputs[i]
        grad = node.op.gradient_as_tuple(node.grad, node)[i]
        if sub_node in list(node_to_output_grads_list.keys()):
          node_to_output_grads_list[sub_node].append(grad)
        else:
          node_to_output_grads_list[sub_node] = [grad]
    ### END YOUR SOLUTION
